...
OUT_BASE = "38_backtest_btcusdt_live_nla_longonly_hedge_hyst05_adx002"
...
def save_plot(eq_df: pd.DataFrame, signal_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    ax0.set_title("38 Study: Live-parity No-lookahead (Long-only + Trend Short Hedge, Hyst 0.5%, ADX=002)")
...
def save_report(
    ...
    lines: list[str] = []
    lines.append("# 38 Backtest: Live Parity (No-lookahead, Long-only + Trend Short Hedge Hyst 0.5%, ADX=002)")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Base target: study-35 logic with ADX calculation replaced by 002 rolling ADX only.")
