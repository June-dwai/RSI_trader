...
OUT_BASE = "49_backtest_btcusdt_scale06_adx002_case1_max_entries_hedge_matched_sweep"
...
def build_max_entries_class(base_cls, max_entries: int):
    class MaxEntriesHedgeMatched(base_cls):
        ...
        def _open_hedge_short(self, price: float, timestamp):
            ...
            # Match hedge size to max_entries for this run (e.g., max_entries=4 -> hedge 4x base qty)
            hedge_qty = base_qty * float(self.max_entries_cap)
            if hedge_qty <= 0:
                return
...
def save_plot(equity_map: dict[int, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("49 Study: Case1(47) Sweep by Max Entries (Hedge Matched)")
...
def save_report(metrics_df: pd.DataFrame, m47):
    ...
    lines: list[str] = []
    lines.append("# 49 Backtest: Study-47 Case1 Max Entries Sweep (Hedge Matched)")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Change from study-48: hedge size is matched to max entries (hedge_multiple = max_entries).")
    ...
    lines.append(f"- Sweep max entries / hedge multiple: `{', '.join(str(x) for x in MAX_ENTRIES_LIST)}`")
    lines.append("")
    ...
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| {int(r['max_entries'])} | {int(r['hedge_multiple'])} | {_fmt(r['final_equity'])} | {_fmt(r['total_return_pct'])} | "
...
def run():
    m47 = load_module("m47_49", BASE_47_PATH)
    ...
    for max_entries in MAX_ENTRIES_LIST:
        ...
        metrics["hedge_multiple"] = int(max_entries)
