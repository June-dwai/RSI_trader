...
SCRIPT_FILE = Path("13_backtest_ethusdt_best_hyst_fixed5x_sl_sweep.py")
PLOT_FILE = Path("13_backtest_ethusdt_best_hyst_fixed5x_sl_sweep.png")
CSV_FILE = Path("13_backtest_ethusdt_best_hyst_fixed5x_sl_sweep.csv")
MD_FILE = Path("13_backtest_ethusdt_best_hyst_fixed5x_sl_sweep.md")
...
def save_plot(equity_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("13 ETHUSDT: 08 Best Hysteresis Fixed5x - SL Sweep")
...
def save_md(metrics_df: pd.DataFrame, best_band: float):
    ...
    lines: list[str] = []
    lines.append("# 13 ETHUSDT - Best Hysteresis Fixed5x SL Sweep")
    lines.append("")
    lines.append("## 1) Objective")
    lines.append("- Test only stop-loss sensitivity on ETH for `08_best_hysteresis_fixed5x` logic.")
