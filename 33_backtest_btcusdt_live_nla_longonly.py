...
OUT_BASE = "33_backtest_btcusdt_live_nla_longonly"
...
class LiveParityNoLookahead:
    """
    Study-33 engine:
    - Live-bot style signal family (entry/DCA/reverse/stop-loss cycle)
...
def save_plot(eq_df: pd.DataFrame, signal_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    ax0.set_title("33 Study: Live-parity No-lookahead (Long-only)")
...
def save_report(
    ...
    lines: list[str] = []
    lines.append("# 33 Backtest: Live Parity (No-lookahead, Long-only)")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Base target: study-32 live-parity no-lookahead engine with strategy shorts blocked.")
