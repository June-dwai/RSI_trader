...
OUT_BASE = "52_backtest_btcusdt_scale06_adx002_case1_m4_partial_hedge_hold_compare"
...
VARIANTS = [
    {"variant": "baseline_close_all", "retain_fraction": 0.00, "stress_gap_pct": 0.00},
    {"variant": "hold10_gap3pct", "retain_fraction": 0.10, "stress_gap_pct": 0.03},
    {"variant": "hold10_gap5pct", "retain_fraction": 0.10, "stress_gap_pct": 0.05},
    {"variant": "hold15_gap5pct", "retain_fraction": 0.15, "stress_gap_pct": 0.05},
    {"variant": "hold20_gap5pct", "retain_fraction": 0.20, "stress_gap_pct": 0.05},
    {"variant": "hold25_gap1pct", "retain_fraction": 0.25, "stress_gap_pct": 0.01},
    {"variant": "hold25_gap2pct", "retain_fraction": 0.25, "stress_gap_pct": 0.02},
    {"variant": "hold25_gap3pct", "retain_fraction": 0.25, "stress_gap_pct": 0.03},
    {"variant": "hold50_gap3pct", "retain_fraction": 0.50, "stress_gap_pct": 0.03},
]
...
def build_variant_class(base_cls, retain_fraction: float, stress_gap_pct: float):
    retain_fraction_value = float(retain_fraction)
    stress_gap_pct_value = float(stress_gap_pct)

    class PartialHedgeHoldCase(base_cls):
        max_entries_cap = MAX_ENTRIES
        hedge_retain_fraction = retain_fraction_value
        stress_gap_pct = stress_gap_pct_value

        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.stats["hedge_reduce_events"] = 0
            self.stats["hedge_topup_events"] = 0
            self.stats["bullish_hold_events"] = 0
        ...
        def _reduce_hedge_short(self, price: float, timestamp, close_qty: float, reason: str):
            ...
            if remaining_qty <= 1e-12:
                ...
                self.stats["hedge_reduce_events"] = int(self.stats.get("hedge_reduce_events", 0)) + 1

        def _is_stressed_long(self, price: float) -> bool:
            ...
            return price <= avg_entry * (1.0 - self.stress_gap_pct)
        ...
        def _manage_trend_hedge(self, confirmed_trend_4h, price: float, timestamp, is_new_4h_bucket: bool):
            ...
            if self.hedge_retain_fraction > 0 and self._is_stressed_long(price):
                desired_qty = self._desired_full_hedge_qty() * self.hedge_retain_fraction
                current_qty = float(self.hedge_position["quantity"])
                if current_qty > desired_qty + 1e-12:
                    self._reduce_hedge_short(price, timestamp, current_qty - desired_qty, "Trend Up Partial")
                    self.stats["bullish_hold_events"] = int(self.stats.get("bullish_hold_events", 0)) + 1
                return

            self._close_hedge_short(price, timestamp, "Trend Up")

    return PartialHedgeHoldCase
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("52 Study: MaxEntries=4 with Partial Hedge Hold Variants + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    baseline = metrics_df[metrics_df["variant"] == "baseline_close_all"].iloc[0]
    improved = metrics_df[
    ...
    lines: list[str] = []
    lines.append("# 52 Backtest: MaxEntries=4 Partial Hedge Hold Variants")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Variant idea: on bullish hedge-close signal, keep part of the hedge only when the long is still full-size and underwater beyond the selected gap threshold.")
    lines.append("")
    lines.append("## Results")
    lines.append("")
    lines.append("| Variant | Retain Fraction | Stress Gap % | Total Final Equity | Total CAGR % | Total MDD % | Total Calmar | Case1 CAGR % | Case1 MDD % | Hedge Reduce | Hedge Top-up | Bullish Hold |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| `{r['variant']}` | {_fmt(r['retain_fraction'] * 100.0)} | {_fmt(r['stress_gap_pct'] * 100.0)} | "
            ...
            f"{int(r['hedge_reduce_events'])} | {int(r['hedge_topup_events'])} | {int(r['bullish_hold_events'])} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append("## Delta vs baseline_close_all")
    ...
    if improved.empty:
        lines.append("- No tested partial-hold variant achieved both `higher total CAGR` and `lower total MDD` than `baseline_close_all`.")
    else:
    ...
    lines.append("- Mild residual hedge can help only if it protects deep stress without choking rebounds too early.")
    lines.append("- Stronger residual hedge is expected to cut drawdown but can reduce CAGR if it stays on through recovery.")
    lines.append("- The key metric is whether any variant improves both total CAGR and total MDD over the `max_entries=4` baseline.")
    lines.append("")
...
def run():
    ...
    for cfg in VARIANTS:
        cls = build_variant_class(m47.LiveParityNoLookahead, cfg["retain_fraction"], cfg["stress_gap_pct"])
        bt = cls(
            symbol=m47.SYMBOL,
        ...
        if not trades_df.empty:
            partial_trades = trades_df[(trades_df["side"] == "SHORT") & (trades_df["reason"] == "Trend Up Partial")]
            trend_up_trades = trades_df[(trades_df["side"] == "SHORT") & (trades_df["reason"].isin(["Trend Up", "Trend Up Partial"]))]
        else:
            partial_trades = pd.DataFrame(columns=["pnl"])
            trend_up_trades = pd.DataFrame(columns=["pnl"])
        ...
        rows.append(
            {
                "variant": cfg["variant"],
                "retain_fraction": cfg["retain_fraction"],
                ...
                "hedge_reduce_events": int(bt.stats.get("hedge_reduce_events", 0)),
                "hedge_topup_events": int(bt.stats.get("hedge_topup_events", 0)),
                "bullish_hold_events": int(bt.stats.get("bullish_hold_events", 0)),
                "trend_up_partial_trades": int(len(partial_trades)),
                "trend_up_partial_net_pnl": float(partial_trades["pnl"].sum()) if len(partial_trades) else 0.0,
                "trend_up_loss_rate": float((trend_up_trades["pnl"] < 0).mean()) if len(trend_up_trades) else np.nan,
            }
        )
