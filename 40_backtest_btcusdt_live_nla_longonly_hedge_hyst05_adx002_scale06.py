...
OUT_BASE = "40_backtest_btcusdt_live_nla_longonly_hedge_hyst05_adx002_scale06"
...
def save_plot(eq_df: pd.DataFrame, signal_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    ax0.set_title("40 Study: Live-parity No-lookahead (Long-only + Trend Short Hedge, Hyst 0.5%, ADX=002)")
...
def save_report(
    ...
    lines: list[str] = []
    lines.append("# 40 Backtest: Live Parity (No-lookahead, Long-only + Trend Short Hedge Hyst 0.5%, ADX=002)")
    lines.append("")
    lines.append("## Setup")
