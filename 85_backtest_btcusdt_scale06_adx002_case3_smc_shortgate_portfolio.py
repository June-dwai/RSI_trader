...
BASE_81_CSV = Path("81_backtest_btcusdt_scale06_adx002_case3_shorttp_portfolio_grid.csv")
CASE3_84_CURVES_CSV = Path("84_backtest_btcusdt_scale06_adx002_smc_short_gate_tune_curves.csv")
CASE3_84_METRICS_CSV = Path("84_backtest_btcusdt_scale06_adx002_smc_short_gate_tune.csv")

OUT_BASE = "85_backtest_btcusdt_scale06_adx002_case3_smc_shortgate_portfolio"
...
def load_study74():
    spec = importlib.util.spec_from_file_location("study74_for_85", BASE_74_PATH)
...
def load_prev_best_81() -> pd.Series:
    df = pd.read_csv(BASE_81_CSV)
...
def load_case3_candidates() -> list[dict]:
    df = pd.read_csv(CASE3_84_METRICS_CSV)
    live = df[~df["variant"].str.startswith("reference_")].copy()
    live = live.sort_values(["calmar_ratio", "cagr_pct"], ascending=[False, False]).reset_index(drop=True)
    picked: list[dict] = []
    for _, row in live.iterrows():
        variant = str(row["variant"])
        if variant.startswith("base"):
            continue
        picked.append({"case3_name": f"{variant}_case3", "variant": variant})
        if len(picked) >= N_CANDIDATES:
            break
    if not picked:
        raise RuntimeError("No live candidate variants found in study 84 metrics.")
    return picked
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame, initial_capital_total: float):
    ...
    ax_eq.set_title("85 Study: SMC Short-Gate Case3 Portfolio")
...
def save_report(metrics_df: pd.DataFrame, baseline70: pd.Series, prev_best_81: pd.Series):
    best = metrics_df.iloc[0]

    lines: list[str] = []
    lines.append("# Study 85: SMC Short-Gate Case3 Portfolio")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Case3 candidates are selected automatically from the top live performers in study 84.")
    lines.append("- Baseline remains the study-70 two-sleeve winner under the same common-period clipping used in this study.")
    lines.append("- Search region stays near the prior winning case1/case2 ridge and keeps case3 small.")
    lines.append("")
    lines.append("## Top 12")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Delta vs Baselines")
    lines.append(
        f"- vs study-70 baseline: CAGR `{_fmt(best['total_cagr_pct'] - baseline70['total_cagr_pct'])}pp`, "
    ...
    lines.append(
        f"- vs study-81 best: CAGR `{_fmt(best['total_cagr_pct'] - prev_best_81['total_cagr_pct'])}pp`, "
        f"MDD `{_fmt(best['total_mdd_pct'] - prev_best_81['total_mdd_pct'])}pp`, "
        f"Calmar `{_fmt(best['total_calmar_ratio'] - prev_best_81['total_calmar_ratio'])}`"
    )
    lines.append("")
    lines.append("## Interpretation")
    if ((metrics_df["total_cagr_pct"] > baseline70["total_cagr_pct"]) & (metrics_df["total_mdd_pct"] < baseline70["total_mdd_pct"])).any():
        lines.append("- At least one study-84 case3 mix dominates the two-sleeve study-70 baseline on both CAGR and MDD.")
    else:
        lines.append("- No study-84 case3 mix dominates the two-sleeve study-70 baseline on both CAGR and MDD.")
    if ((metrics_df["total_cagr_pct"] > prev_best_81["total_cagr_pct"]) & (metrics_df["total_mdd_pct"] < prev_best_81["total_mdd_pct"])).any():
        lines.append("- At least one study-84 case3 mix also dominates the prior study-81 best case3 mix.")
    else:
        lines.append("- The study-84 case3 candidates improve some metrics but do not fully dominate the prior study-81 best.")
    lines.append("- If the best case3 weight stays around `5%~7%`, this sleeve is still acting as a diversifier rather than a core engine.")
    lines.append("")
...
def run():
    ...
    candidates = load_case3_candidates()
    ...
    prev_best_81 = load_prev_best_81()

    rows: list[dict] = []
    curve_map: dict[str, pd.DataFrame] = {}

    baseline_case3 = s74.load_case3(CASE3_84_CURVES_CSV, candidates[0]["variant"], candidates[0]["case3_name"])
    ...
    baseline_out, baseline_run = s74.run_three_sleeve(baseline_merged, candidates[0]["case3_name"], 0.74, 0.26, 0.00)
    ...
    for candidate in candidates:
        ...
        case3 = s74.load_case3(CASE3_84_CURVES_CSV, str(candidate["variant"]), case3_name)
    ...
    save_report(metrics_df, baseline70, prev_best_81)
