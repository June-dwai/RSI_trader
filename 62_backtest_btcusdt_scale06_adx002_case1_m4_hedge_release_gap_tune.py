...
OUT_BASE = "62_backtest_btcusdt_scale06_adx002_case1_m4_hedge_release_gap_tune"
...
VARIANTS = [
    ...
    {"variant": "shallow4_else2bull", "bullish_close_bars": 2, "shallow_gap_pct": 0.04},
    {"variant": "shallow5_else2bull", "bullish_close_bars": 2, "shallow_gap_pct": 0.05},
    {"variant": "shallow6_else2bull", "bullish_close_bars": 2, "shallow_gap_pct": 0.06},
    {"variant": "shallow7_else2bull", "bullish_close_bars": 2, "shallow_gap_pct": 0.07},
]
...
def build_variant_class(base_cls, bullish_close_bars: int, shallow_gap_pct: float):
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("62 Study: MaxEntries=4 Hedge Release Gap Tuning + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 62 Backtest: MaxEntries=4 Hedge Release Gap Tuning")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Variant idea: use the best 61-structure (`2 bullish bars unless the long is only shallowly underwater`) and tune the shallow-release gap itself.")
    lines.append("")
    lines.append("## Results")
    lines.append("")
    lines.append("| Variant | Bullish Close Bars | Shallow Gap % | Total Final Equity | Total CAGR % | Total MDD % | Total Calmar | Case1 CAGR % | Case1 MDD % | Bullish Hold | Early Release |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in metrics_df.iterrows():
        lines.append(
            ...
            f"{int(r['bullish_hold_bars'])} | {int(r['early_release_events'])} |"
        )
    lines.append("")
    ...
    lines.append("- If a variant helps, it means hedge losses were being realized too early on single-bucket bullish flips.")
    lines.append("- Waiting too long to release hedge will drag rebounds, so the tradeoff is whipsaw protection versus recovery participation.")
    lines.append("- The key metric is whether there is a shallow-gap sweet spot that preserves the 61 CAGR gain without giving back total MDD.")
    lines.append("")
...
def run():
    ...
    for cfg in VARIANTS:
        cls = build_variant_class(m47.LiveParityNoLookahead, cfg["bullish_close_bars"], cfg["shallow_gap_pct"])
        bt = cls(
            symbol=m47.SYMBOL,
