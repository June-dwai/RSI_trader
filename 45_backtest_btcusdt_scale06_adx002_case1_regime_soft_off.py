...
OUT_BASE = "45_backtest_btcusdt_scale06_adx002_case1_regime_soft_off"
...
REGIME_CASES = [
    RegimeCase(
        regime_id="r1_bear_only",
        label="R1 bear_only",
        off_rule="trend_4h_confirmed == bearish",
        on_rule="trend_4h_confirmed == bullish and run_len_4h >= 2",
    ),
    RegimeCase(
        regime_id="r2_bear_mature",
        label="R2 bear_mature",
        off_rule="trend_4h_confirmed == bearish and run_len_4h >= 12",
        on_rule="trend_4h_confirmed == bullish and run_len_4h >= 2",
    ),
    RegimeCase(
        regime_id="r3_bear_chop",
        label="R3 bear_chop",
        off_rule="trend_4h_confirmed == bearish and flip_count_30_4h >= 2",
        on_rule="(trend_4h_confirmed == bullish and run_len_4h >= 2) or flip_count_30_4h <= 1",
    ),
    RegimeCase(
        regime_id="r4_bear_highvol_p80",
        label="R4 bear_highvol(P80)",
        off_rule="trend_4h_confirmed == bearish and vol20_confirmed >= vol20_p80_2y",
        on_rule="trend_4h_confirmed == bullish and vol20_confirmed <= vol20_p65_2y",
    ),
    RegimeCase(
        regime_id="r5_risk_or_crash",
        label="R5 risk_or_crash",
        off_rule="risk_score_24 >= 4 or (trend_4h_confirmed == bearish and dd30_confirmed <= -6)",
        on_rule="risk_score_24 <= 2 and ((trend_4h_confirmed == bullish and run_len_4h >= 2) or dd30_confirmed >= -3)",
    ),
]

REGIME_ORDER = [x.regime_id for x in REGIME_CASES]
REGIME_LABELS = {x.regime_id: x.label for x in REGIME_CASES}
REGIME_OFF_RULE = {x.regime_id: x.off_rule for x in REGIME_CASES}
REGIME_ON_RULE = {x.regime_id: x.on_rule for x in REGIME_CASES}
...
def build_case1_regime_class(case1_base_mod, regime_id: str):
    ...
    class Case1RegimeSoftOff(BaseCls):
        ...
        def _off_condition(self, row: pd.Series) -> tuple[bool, str]:
            ...
            flip30 = _safe_float(row.get("flip_count_30_4h", np.nan))
            vol20 = _safe_float(row.get("vol20_confirmed", np.nan))
            vol_p80 = _safe_float(row.get("vol20_p80_2y", np.nan))
            ...
            if self.regime_id == "r1_bear_only":
                cond = trend == "bearish"
                return bool(cond), "bearish trend"

            if self.regime_id == "r2_bear_mature":
                cond = (trend == "bearish") and pd.notna(run_len) and run_len >= 12
                return bool(cond), "bearish mature run (>=12)"

            if self.regime_id == "r3_bear_chop":
                cond = (trend == "bearish") and pd.notna(flip30) and flip30 >= 2
                return bool(cond), "bearish + chop flip30>=2"

            if self.regime_id == "r4_bear_highvol_p80":
                threshold = vol_p80 if pd.notna(vol_p80) else VOL_P80_FALLBACK
                cond = (trend == "bearish") and pd.notna(vol20) and (vol20 >= threshold)
                return bool(cond), f"bearish + vol20>=P80 ({_fmt(threshold, 3)})"

            if self.regime_id == "r5_risk_or_crash":
                cond = (risk_score >= 4) or ((trend == "bearish") and pd.notna(dd30) and (dd30 <= DD_CRASH_OFF))
                return bool(cond), "risk_score>=4 or bearish crash dd30<=-6"
        ...
        def _on_condition(self, row: pd.Series) -> tuple[bool, str]:
            ...
            flip30 = _safe_float(row.get("flip_count_30_4h", np.nan))
            vol20 = _safe_float(row.get("vol20_confirmed", np.nan))
            vol_p65 = _safe_float(row.get("vol20_p65_2y", np.nan))
            risk_score = int(row.get("risk_score_24", 0)) if pd.notna(row.get("risk_score_24", np.nan)) else 0
            dd30 = _safe_float(row.get("dd30_confirmed", np.nan))

            if self.regime_id in ("r1_bear_only", "r2_bear_mature"):
                cond = (trend == "bullish") and pd.notna(run_len) and run_len >= 2
                return bool(cond), "bullish trend re-confirmed (>=2 bars)"

            if self.regime_id == "r3_bear_chop":
                cond = ((trend == "bullish") and pd.notna(run_len) and run_len >= 2) or (pd.notna(flip30) and flip30 <= 1)
                return bool(cond), "chop eased (flip30<=1) or bullish reconfirmed"

            if self.regime_id == "r4_bear_highvol_p80":
                threshold = vol_p65 if pd.notna(vol_p65) else VOL_P65_FALLBACK
                cond = (trend == "bullish") and pd.notna(vol20) and (vol20 <= threshold)
                return bool(cond), f"bullish + vol20<=P65 ({_fmt(threshold, 3)})"

            if self.regime_id == "r5_risk_or_crash":
                cond = (risk_score <= 2) and (
                    ((trend == "bullish") and pd.notna(run_len) and run_len >= 2)
                    or (pd.notna(dd30) and dd30 >= DD_CRASH_ON)
                )
                return bool(cond), "risk_score<=2 and trend/crash eased"
...
def save_plot(total_map: dict[str, pd.DataFrame], case1_map: dict[str, pd.DataFrame], eq_case2: pd.DataFrame):
    palette = {
        "r1_bear_only": "#1f77b4",
        "r2_bear_mature": "#ff7f0e",
        "r3_bear_chop": "#2ca02c",
        "r4_bear_highvol_p80": "#d62728",
        "r5_risk_or_crash": "#9467bd",
    }

    fig, axes = plt.subplots(3, 1, figsize=(16, 12), sharex=True, gridspec_kw={"height_ratios": [1.2, 1.0, 1.0]})
    ax0, ax1, ax2 = axes

    for v in REGIME_ORDER:
        tc = total_map[v]
        ax0.plot(tc["timestamp"], tc["equity_total"], linewidth=1.05, color=palette[v], label=f"Total ({REGIME_LABELS[v]})")
    ax0.axhline(INITIAL_CAPITAL_EACH * 2.0, color="#777777", linestyle="--", linewidth=0.9, label="Start 2000")
    ax0.set_title("45 Study: Case1 Regime Soft-OFF (5 variants) + Fixed Case2")
    ...
    for v in REGIME_ORDER:
        eq = case1_map[v]
        ax1.plot(eq["timestamp"], eq["equity"], linewidth=1.05, color=palette[v], label=REGIME_LABELS[v])
...
def save_report(metrics_df: pd.DataFrame, case1_df: pd.DataFrame, stress_df: pd.DataFrame):
    regime_def_df = pd.DataFrame(
        {
            "regime_id": REGIME_ORDER,
            "label": [REGIME_LABELS[x] for x in REGIME_ORDER],
            "off_rule": [REGIME_OFF_RULE[x] for x in REGIME_ORDER],
            "on_rule": [REGIME_ON_RULE[x] for x in REGIME_ORDER],
            "off_to_on_cooldown_4h_bars": OFF_MIN_BARS_4H,
        }
    )

    lines: list[str] = []
    lines.append("# 45 백테스트: Case1 Regime Soft-OFF 5종 비교 (Case2 고정)")
    lines.append("")
    lines.append("## 구성")
    lines.append("- 기본 구조: `Case1 + Case2` 합산 Total.")
    lines.append("- Case1은 study-40 baseline(hedge5x, scale0.60) 유지.")
    lines.append("- Case2는 study-42 설정 그대로 고정.")
    lines.append("- 실험 변수는 `Case1 regime ON/OFF`만 변경 (5개).")
    lines.append("")
    lines.append("## OFF 동작(요청 반영)")
    lines.append("- `Soft OFF`: 신규 진입 중지 + DCA/REENTRY 중지 + 신규 hedge open 중지.")
    ...
    lines.append("- ON 조건은 OFF 조건보다 보수적으로 구성(히스테리시스).")
    lines.append("")
    lines.append("## Regime 정의")
    lines.extend(df_to_md_table(regime_def_df))
    lines.append("")
    lines.append("## 성과 요약")
    lines.extend(df_to_md_table(metrics_df))
    lines.append("")
    lines.append("## Case1 + Regime 상태 통계")
...
def run():
    base = load_module("m002_45", BASE_002_PATH)
    helper = load_module("m04_45", BASE_04_PATH)
    m40 = load_module("m40_45", CASE1_BASE_PATH)
    m32 = load_module("m32_45", CASE2_BASE_PATH)
    m42 = load_module("m42_45", CASE2_WRAPPER_PATH)
    ...
    print(f"saved_case1_regime_stats={OUT_CASE1_REGIME_CSV}")
    print(f"saved_regime_events={OUT_REGIME_EVENTS_CSV}")
