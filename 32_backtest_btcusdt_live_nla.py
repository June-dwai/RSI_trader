...
OUT_BASE = "32_backtest_btcusdt_live_nla"
...
def save_plot(eq_df: pd.DataFrame, signal_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    ax0.set_title("32 Study: Live-parity Logic with Strict No-lookahead")
...
def save_report(
    ...
    lines: list[str] = []
    lines.append("# 32 Backtest: Live Parity (No-lookahead Strict)")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Base target: `live_rsi_bot.py` signal family (entry/DCA/reverse/stop-loss cycle).")
