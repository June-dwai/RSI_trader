...
OUT_BASE = "47_backtest_btcusdt_scale06_adx002_case1_standalone"
...
def load_data_no_filter() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_1m = _load_cached_df(SYMBOL, "1m", periods_1m).sort_index()
    df_4h = _load_cached_df(SYMBOL, "4h", periods_4h).sort_index()
    return df_1m, df_4h
...
class LiveParityNoLookahead:
    ...
    - Add trend short hedge (5x base qty) using confirmed 4h hysteresis trend (0.5%)
...
def configure_baseline_params(bt: LiveParityNoLookahead):
    bt.rsi_oversold = 18
...
def calculate_metrics(bt: LiveParityNoLookahead, initial_capital: float) -> dict:
...
def save_plot(eq_df: pd.DataFrame, signal_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    ax0.set_title("47 Study: Live-parity No-lookahead (Long-only + Trend Short Hedge, Hyst 0.5%, ADX=002)")
...
def save_report(
    ...
    lines: list[str] = []
    lines.append("# 47 Backtest: Live Parity (No-lookahead, Long-only + Trend Short Hedge Hyst 0.5%, ADX=002)")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Base target: study-38 logic with entry scale changed to 0.60.")
...
def run():
    df_1m, df_4h = load_data_no_filter()
    ...
    print(
        f"symbol={SYMBOL}, final_equity={_fmt(metrics.get('final_equity'))}, "
