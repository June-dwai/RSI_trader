...
OUT_BASE = "39_backtest_btcusdt_live_nla_longonly_hedge_hyst05_adx002_nosl"
...
class LiveParityNoLookahead:
    ...
    - Disable stop-loss / re-entry cycle entirely
    ...
    def _check_stop_loss_and_reentry(self, price: float, timestamp):
        # Study-39: stop-loss/re-entry cycle intentionally disabled.
        self.stop_loss = [0.0, 0.0]
        self.pending_reentry = None
...
def save_plot(eq_df: pd.DataFrame, signal_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    ax0.set_title("39 Study: Live-parity No-lookahead (Long-only + Trend Short Hedge, No SL, Hyst 0.5%, ADX=002)")
...
def save_report(
    ...
    lines: list[str] = []
    lines.append("# 39 Backtest: Live Parity (No-lookahead, Long-only + Trend Short Hedge, No Stop-loss, ADX=002)")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Base target: study-38 logic with stop-loss/re-entry cycle removed.")
    ...
    lines.append("- Parameters: RSI(6), oversold 18, overbought 85, TP 1.2%, stop-loss disabled, max position 5x.")
    ...
    lines.append("- Stop-loss / re-entry: `disabled`")
...
def run():
    base = load_module("m002_39", BASE_002_PATH)
    helper = load_module("m04_39", BASE_04_PATH)
