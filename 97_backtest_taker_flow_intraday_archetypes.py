...
OUT_BASE = "97_backtest_taker_flow_intraday_archetypes"
...
VARIANTS = [
    {"variant": "flow_reclaim_dual_24h_x100", "entry_type": "reclaim", "lookback_bars": 24, "hold_bars": 6, "stop_atr": 1.0, "tp_atr": 1.8, "exposure": 1.0},
    {"variant": "flow_reclaim_dual_24h_x125", "entry_type": "reclaim", "lookback_bars": 24, "hold_bars": 6, "stop_atr": 1.0, "tp_atr": 1.8, "exposure": 1.25},
    {"variant": "flow_follow_dual_24h_x100", "entry_type": "follow", "lookback_bars": 24, "hold_bars": 6, "stop_atr": 1.0, "tp_atr": 2.0, "exposure": 1.0},
    {"variant": "flow_follow_dual_48h_x100", "entry_type": "follow", "lookback_bars": 48, "hold_bars": 8, "stop_atr": 1.0, "tp_atr": 2.5, "exposure": 1.0},
]
...
def build_market_df() -> pd.DataFrame:
    ...
    for col in ["open", "high", "low", "close", "volume", "taker_base_vol"]:
        btc_1m[col] = pd.to_numeric(btc_1m[col], errors="coerce")
    df1h = btc_1m.resample("1h").agg(
        {
            "open": "first",
            "high": "max",
            "low": "min",
    ...
    tr = pd.concat(
        [
            df1h["high"] - df1h["low"],
            (df1h["high"] - prev_close).abs(),
            (df1h["low"] - prev_close).abs(),
        ],
        axis=1,
    ).max(axis=1)
    df1h["atr24"] = tr.rolling(24).mean()
    df1h["ret_close"] = df1h["close"].pct_change().fillna(0.0)
    df1h["buy_ratio"] = (df1h["taker_base_vol"] / df1h["volume"].replace(0, np.nan)).clip(0.0, 1.0)
    df1h["taker_imbalance"] = (df1h["buy_ratio"] * 2.0) - 1.0
    df1h["log_volume"] = np.log(df1h["volume"].clip(lower=1.0))
    df1h["volume_z"] = (df1h["log_volume"] - df1h["log_volume"].rolling(48).mean()) / df1h["log_volume"].rolling(48).std()

    df4h = btc_1m.resample("4h").agg({"open": "first", "high": "max", "low": "min", "close": "last"}).dropna()
    ...
    merged = pd.merge_asof(
        df1h.reset_index().sort_values("timestamp"),
...
def run_variant(df: pd.DataFrame, cfg: dict) -> tuple[pd.DataFrame, dict]:
    ...
    work["prev_high"] = work["high"].rolling(lookback).max().shift(1)
    work["prev_low"] = work["low"].rolling(lookback).min().shift(1)
    ...
    for i in range(1, len(work)):
        ...
        atr_i = float(work.loc[i, "atr24"])
        trend_i = work.loc[i, "trend_4h_confirmed"]
        buy_ratio = float(work.loc[i, "buy_ratio"])
        vol_z = float(work.loc[i, "volume_z"])
        ...
        if current_side == 0:
            ...
            if cfg["entry_type"] == "reclaim":
                long_signal = (
                    trend_i == "bullish"
                    and float(work.loc[i, "low"]) < prev_low
                    and close_i > prev_low
                    and buy_ratio <= 0.42
                    and vol_z >= 0.75
                )
                short_signal = (
                    trend_i == "bearish"
                    and float(work.loc[i, "high"]) > prev_high
                    and close_i < prev_high
                    and buy_ratio >= 0.58
                    and vol_z >= 0.75
                )
            else:
                long_signal = trend_i == "bullish" and close_i > prev_high and buy_ratio >= 0.58 and vol_z >= 0.75
                short_signal = trend_i == "bearish" and close_i < prev_low and buy_ratio <= 0.42 and vol_z >= 0.75
...
def save_plot(metrics_df: pd.DataFrame, curve_map: dict[str, pd.DataFrame]) -> None:
    ...
    ax_eq.set_title("97번 연구: Taker Flow Intraday Archetypes")
...
def save_report(metrics_df: pd.DataFrame) -> None:
    ...
    lines: list[str] = []
    lines.append("# 97번 연구: Taker Flow Intraday Archetypes")
    lines.append("")
    lines.append("## 설정")
    lines.append("- BTC 1시간봉 taker flow와 4시간 confirmed trend를 결합한 standalone 구조다.")
    lines.append("- reclaim은 liquidity sweep 후 aggressive taker flow를 확인하고 되돌림을 먹는다.")
    lines.append("- follow는 aggressive taker flow가 돌파를 동반할 때 추세를 따라간다.")
    lines.append("")
    lines.append("## 결과")
    lines.append("")
    ...
    lines.append("- 이 연구는 taker-flow 자체만으로 standalone 알파가 있는지 확인하는 목적이다.")
    lines.append("")
    lines.append("## 산출물")
