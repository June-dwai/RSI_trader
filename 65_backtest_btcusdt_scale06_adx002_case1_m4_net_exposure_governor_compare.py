...
OUT_BASE = "65_backtest_btcusdt_scale06_adx002_case1_m4_net_exposure_governor_compare"
...
VARIANTS = [
    {"variant": "release1bull", "bullish_close_bars": 1, "shallow_gap_pct": -1.0},
    {"variant": "shallow6_else2bull", "bullish_close_bars": 2, "shallow_gap_pct": 0.06},
    {"variant": "gov_net2p5_gap3", "bullish_close_bars": 2, "shallow_gap_pct": 0.06, "target_net_entries": 2.5, "governor_gap_pct": 0.03},
    {"variant": "gov_net2p0_gap3", "bullish_close_bars": 2, "shallow_gap_pct": 0.06, "target_net_entries": 2.0, "governor_gap_pct": 0.03},
    {"variant": "gov_net2p0_gap5", "bullish_close_bars": 2, "shallow_gap_pct": 0.06, "target_net_entries": 2.0, "governor_gap_pct": 0.05},
    {"variant": "gov_net1p5_gap5", "bullish_close_bars": 2, "shallow_gap_pct": 0.06, "target_net_entries": 1.5, "governor_gap_pct": 0.05},
]
...
def build_variant_class(base_cls, bullish_close_bars: int, shallow_gap_pct: float, target_net_entries: float, governor_gap_pct: float):
    bullish_close_bars_value = int(bullish_close_bars)
    shallow_gap_pct_value = float(shallow_gap_pct)
    target_net_entries_value = float(target_net_entries)
    governor_gap_pct_value = float(governor_gap_pct)

    class HedgeReleaseDelayCase(base_cls):
        max_entries_cap = MAX_ENTRIES
        dca_drop_pct = DCA_DROP_PCT
        bullish_close_bars = bullish_close_bars_value
        shallow_gap_pct = shallow_gap_pct_value
        target_net_entries = target_net_entries_value
        governor_gap_pct = governor_gap_pct_value

        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.stats["hedge_topup_events"] = 0
            self.stats["dca_signal_events"] = 0
            self.stats["bullish_hold_bars"] = 0
            self.stats["early_release_events"] = 0
            self.stats["governor_reduce_events"] = 0
            self.stats["governor_close_events"] = 0
            self.bullish_release_streak = 0
        ...
        def _reduce_hedge_short(self, price: float, timestamp, close_qty: float, reason: str):
            if self.hedge_position is None:
                return
            pos = self.hedge_position
            qty = min(close_qty, pos["quantity"])
            if qty <= 0:
                return

            original_qty = float(pos["quantity"])
            open_commission_alloc = float(pos["total_commission"]) * (qty / max(original_qty, 1e-12))
            close_commission = qty * price * self.commission
            pnl = (pos["avg_entry"] - price) * qty - (open_commission_alloc + close_commission)
            self.capital += pnl
            ...
            self._mark_order(timestamp, price, "BUY", qty, "HEDGE_REDUCE")

            remaining_qty = original_qty - qty
            if remaining_qty <= 1e-12:
                self.hedge_position = None
                self.stats["hedge_close_events"] += 1
                self.stats["governor_close_events"] = int(self.stats.get("governor_close_events", 0)) + 1
            else:
                pos["quantity"] = float(remaining_qty)
                pos["total_commission"] = float(pos["total_commission"] - open_commission_alloc)
                self.stats["governor_reduce_events"] = int(self.stats.get("governor_reduce_events", 0)) + 1

        def _is_governor_stressed_long(self, price: float) -> bool:
            if self.current_position is None or self.current_position["side"] != "LONG":
                return False
            avg_entry = float(self.current_position["avg_entry"])
            return price <= avg_entry * (1.0 - self.governor_gap_pct)

        def _desired_governor_hedge_qty(self) -> float:
            if self.current_position is None or self.current_position["side"] != "LONG" or self.position_quantity <= 0:
                return 0.0
            current_long_qty = float(self.current_position["quantity"])
            target_net_qty = float(self.position_quantity) * self.target_net_entries
            return max(0.0, current_long_qty - target_net_qty)
        ...
        def _manage_trend_hedge(self, confirmed_trend_4h, price: float, timestamp, is_new_4h_bucket: bool):
            ...
            self.bullish_release_streak += 1
            if self.shallow_gap_pct >= 0 and self.current_position and self.current_position["side"] == "LONG":
                avg_entry = float(self.current_position["avg_entry"])
                if price >= avg_entry * (1.0 - self.shallow_gap_pct):
                    self._close_hedge_short(price, timestamp, "Trend Up EarlyRelease")
                    self.bullish_release_streak = 0
                    self.stats["early_release_events"] = int(self.stats.get("early_release_events", 0)) + 1
                    return

            if self.bullish_release_streak >= self.bullish_close_bars:
                if self.target_net_entries < self.max_entries_cap and self._is_governor_stressed_long(price):
                    desired_qty = self._desired_governor_hedge_qty()
                    current_qty = float(self.hedge_position["quantity"])
                    if current_qty > desired_qty + 1e-12:
                        self._reduce_hedge_short(price, timestamp, current_qty - desired_qty, "Trend Up Governor")
                    self.bullish_release_streak = 0
                    self.stats["bullish_hold_bars"] = int(self.stats.get("bullish_hold_bars", 0)) + 1
                    return
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("65 Study: MaxEntries=4 Net Exposure Governor Variants + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    baseline = metrics_df[metrics_df["variant"] == "release1bull"].iloc[0]
    improved = metrics_df[
    ...
    lines: list[str] = []
    lines.append("# 65 Backtest: MaxEntries=4 Net Exposure Governor Variants")
    lines.append("")
    lines.append("## Setup")
    lines.append("- `case1` uses study-51 baseline: `max_entries=4`, matched hedge size, original `dca_drop=0.50%`.")
    lines.append("- `case2` stays fixed as study-42 case2 curve.")
    lines.append("- Base candidate is `shallow6_else2bull`: wait `2` bullish 4h buckets unless the long is already within `6%` of average entry.")
    lines.append("- Governor variants keep that release logic, but if the long is still deeply underwater they only reduce hedge to a target net-long size instead of closing hedge fully.")
    lines.append("")
    lines.append("## Results")
    lines.append("")
    lines.append("| Variant | Bullish Close Bars | Shallow Gap % | Target Net Entries | Governor Gap % | Total Final Equity | Total CAGR % | Total MDD % | Total Calmar | Case1 CAGR % | Case1 MDD % | Governor Reduce | Governor Close |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| `{r['variant']}` | {int(r['bullish_close_bars'])} | {_fmt(max(r['shallow_gap_pct'], 0.0) * 100.0)} | "
            f"{_fmt(r['target_net_entries'])} | {_fmt(r['governor_gap_pct'] * 100.0)} | "
            ...
            f"{int(r['governor_reduce_events'])} | {int(r['governor_close_events'])} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append("## Delta vs release1bull")
    ...
    if improved.empty:
        lines.append("- No tested hedge-release-delay variant achieved both `higher total CAGR` and `lower total MDD` than `release1bull`.")
    else:
    ...
    lines.append("- If a variant helps, it means the remaining issue after study 62 is not hedge timing alone but over-restoring net-long exposure while still deeply underwater.")
    lines.append("- If governor variants fail, it means keeping residual hedge into recovery is choking the same rebound engine that makes case1 profitable.")
    lines.append("- The key metric is whether target net exposure can preserve the study-62 CAGR gain while pulling MDD back toward the original baseline.")
    lines.append("")
...
def run():
    ...
    for cfg in VARIANTS:
        cls = build_variant_class(
            m47.LiveParityNoLookahead,
            cfg["bullish_close_bars"],
            cfg["shallow_gap_pct"],
            float(cfg.get("target_net_entries", MAX_ENTRIES)),
            float(cfg.get("governor_gap_pct", 0.0)),
        )
        bt = cls(
            symbol=m47.SYMBOL,
        ...
        rows.append(
            {
                "variant": cfg["variant"],
                "bullish_close_bars": cfg["bullish_close_bars"],
                "shallow_gap_pct": cfg["shallow_gap_pct"],
                "target_net_entries": float(cfg.get("target_net_entries", MAX_ENTRIES)),
                "governor_gap_pct": float(cfg.get("governor_gap_pct", 0.0)),
                ...
                "bullish_hold_bars": int(bt.stats.get("bullish_hold_bars", 0)),
                "early_release_events": int(bt.stats.get("early_release_events", 0)),
                "governor_reduce_events": int(bt.stats.get("governor_reduce_events", 0)),
                "governor_close_events": int(bt.stats.get("governor_close_events", 0)),
            }
        )
