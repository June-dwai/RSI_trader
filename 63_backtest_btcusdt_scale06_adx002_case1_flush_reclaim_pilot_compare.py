...
OUT_BASE = "63_backtest_btcusdt_scale06_adx002_case1_flush_reclaim_pilot_compare"
...
VARIANTS = [
    ...
    {
        "variant": "flush0p75_reclaim0p25_p15_step0p75",
        "mode": "structural",
        "flush_drop_pct": 0.0075,
        "reclaim_pct": 0.0025,
        "setup_wait_bars": 240,
        "pilot_entry_scale": 0.15,
        "pyramid_step_pct": 0.0075,
    },
    {
        "variant": "flush1p00_reclaim0p25_p15_step0p75",
        "mode": "structural",
        "flush_drop_pct": 0.0100,
        "reclaim_pct": 0.0025,
        "setup_wait_bars": 240,
        "pilot_entry_scale": 0.15,
        "pyramid_step_pct": 0.0075,
    ...
    {
        "variant": "flush1p00_reclaim0p50_p15_step1p00",
        "mode": "structural",
        "flush_drop_pct": 0.0100,
        "reclaim_pct": 0.0050,
        "setup_wait_bars": 360,
        "pilot_entry_scale": 0.15,
        "pyramid_step_pct": 0.0100,
    },
    {
        "variant": "flush0p75_reclaim0p25_p20_step0p75",
        "mode": "structural",
        "flush_drop_pct": 0.0075,
        "reclaim_pct": 0.0025,
        "setup_wait_bars": 240,
        "pilot_entry_scale": 0.20,
        "pyramid_step_pct": 0.0075,
    },
]
...
def build_variant_class(base_module, base_cls, cfg: dict):
    ...
    flush_drop_pct_value = float(cfg.get("flush_drop_pct", 0.0))
    reclaim_pct_value = float(cfg.get("reclaim_pct", 0.0))
    setup_wait_bars_value = int(cfg.get("setup_wait_bars", 0))
    pilot_entry_scale_value = float(cfg.get("pilot_entry_scale", ENTRY_SCALE))
    pyramid_step_pct_value = float(cfg.get("pyramid_step_pct", 0.0))
    ...
    class StructuralCase(base_cls):
        max_entries_cap = MAX_ENTRIES
        dca_drop_pct = dca_drop_pct_value
        flush_drop_pct = flush_drop_pct_value
        reclaim_pct = reclaim_pct_value
        setup_wait_bars = setup_wait_bars_value
        pilot_entry_scale = pilot_entry_scale_value
        pyramid_step_pct = pyramid_step_pct_value
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.stats["setup_created"] = 0
            self.stats["setup_flushed"] = 0
            self.stats["setup_filled"] = 0
            self.stats["setup_cancelled"] = 0
            self.stats["pyramid_add_events"] = 0
            self.stats["hard_stop_events"] = 0
            self.stats["lower_reentry_blocked"] = 0
            self.pending_long_setup: dict | None = None
        ...
        def _place_long_setup(self, price: float, timestamp, current_time: int):
            self.pending_long_setup = {
                "signal_price": float(price),
                "created_time": pd.to_datetime(timestamp),
                "created_idx": int(current_time),
                "flush_trigger": float(price * (1.0 - self.flush_drop_pct)),
                "flushed": False,
                "flush_low_close": float(price),
            }
            self.stats["setup_created"] = int(self.stats.get("setup_created", 0)) + 1

        def _cancel_long_setup(self):
            if self.pending_long_setup is None:
                return
            self.pending_long_setup = None
            self.stats["setup_cancelled"] = int(self.stats.get("setup_cancelled", 0)) + 1

        def _process_long_setup(self, price: float, timestamp, current_time: int, trend: str, ema_touch: bool):
            if self.pending_long_setup is None:
                return
            ...
            setup = self.pending_long_setup
            if ema_touch or trend != "bullish":
                self._cancel_long_setup()
                return
            if (current_time - int(setup["created_idx"])) > self.setup_wait_bars:
                self._cancel_long_setup()
                return

            if price < float(setup["flush_low_close"]):
                setup["flush_low_close"] = float(price)
            if (not bool(setup["flushed"])) and price <= float(setup["flush_trigger"]):
                setup["flushed"] = True
                setup["flush_low_close"] = float(price)
                self.stats["setup_flushed"] = int(self.stats.get("setup_flushed", 0)) + 1

            if not bool(setup["flushed"]):
                return

            reclaim_trigger = float(setup["flush_low_close"]) * (1.0 + self.reclaim_pct)
            if price < reclaim_trigger:
                return
            if self.capital <= 0:
                self.pending_long_setup = None
                return

            qty = (self.capital / price) * self.pilot_entry_scale
            self._open_position("LONG", price, timestamp, qty, "OPEN_FLUSH_RECLAIM")
            self.last_order_time = current_time
            self.pending_long_setup = None
            self.stats["setup_filled"] = int(self.stats.get("setup_filled", 0)) + 1

        def _process_long_pyramid(self, price: float, timestamp, current_time: int, trend: str):
            if not self.current_position:
                return
            if self.current_position["side"] != "LONG":
                return
            if self.entry_count >= self.max_entries_cap:
                return
            if trend != "bullish":
                return

            trigger_price = float(self.recent_trade[0]) * (1.0 + self.pyramid_step_pct)
            if price < trigger_price:
                return

            before_qty = float(self.current_position["quantity"])
            self._add_to_position(price, timestamp, self.position_quantity, "PYRAMID")
            if self.current_position and float(self.current_position["quantity"]) > before_qty + 1e-12:
                self.last_order_time = current_time
                self.stats["pyramid_add_events"] = int(self.stats.get("pyramid_add_events", 0)) + 1
        ...
        def _check_stop_loss_and_reentry(self, price: float, timestamp):
            ...
            stop_price = float(pos["avg_entry"]) * (1.0 - self.stop_loss_pct)
            if price <= stop_price:
                self._close_position(price, timestamp, "Hard Stop")
                self.pending_reentry = None
                self.stop_loss = [0.0, 0.0]
                self.stats["hard_stop_events"] = int(self.stats.get("hard_stop_events", 0)) + 1
        ...
        def run(self, df_1m: pd.DataFrame, df_4h: pd.DataFrame, backtest_start_date=None):
            ...
            for i in range(max(ema_period_value, 200), len(out_1m)):
                ...
                self._process_long_setup(price, timestamp, i, trend, ema_touch)
                self._process_long_pyramid(price, timestamp, i, trend)
                ...
                if (not ema_touch) and time_since_last >= self.cooldown_time:
                    if rsi <= self.rsi_oversold and trend == "bullish":
                        ...
                        if self.current_position is None and self.pending_long_setup is None:
                            self._place_long_setup(price, timestamp, i)
                        elif self.current_position is not None:
                            self.stats["lower_reentry_blocked"] = int(self.stats.get("lower_reentry_blocked", 0)) + 1
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("63 Study: Flush/Reclaim Pilot Entry + Winner Pyramid Variants + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 63 Backtest: Flush/Reclaim Pilot Entry + Winner Pyramid Variants")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Structural rows replace loser averaging with: `flush -> reclaim -> pilot fill`, then `winner pyramid` adds, plus `hard stop without lower reentry`.")
    lines.append("- No lookahead rule: structural setup state uses only sequential 1-minute `close` observations. It does not infer low-before-high order inside the same bar.")
    lines.append("")
    lines.append("## Results")
    lines.append("")
    lines.append("| Variant | Mode | Flush % | Reclaim % | Pilot Scale | Pyramid Step % | Total Final Equity | Total CAGR % | Total MDD % | Total Calmar | Case1 CAGR % | Case1 MDD % | Setups Filled | Pyramid Adds | Hard Stops |")
    lines.append("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| `{r['variant']}` | `{r['mode']}` | {_fmt(r['flush_drop_pct'] * 100.0)} | {_fmt(r['reclaim_pct'] * 100.0)} | "
            f"{_fmt(r['pilot_entry_scale'] * 100.0)} | {_fmt(r['pyramid_step_pct'] * 100.0)} | "
            ...
            f"{int(r['setup_filled'])} | {int(r['pyramid_add_events'])} | {int(r['hard_stop_events'])} |"
        )
    lines.append("")
    ...
    if improved.empty:
        lines.append("- No tested structural variant achieved both `higher total CAGR` and `lower total MDD` than `baseline_m4`.")
    else:
    ...
    lines.append("- If a structural row helps, it means the main damage came from building size into weakness rather than from the long idea itself.")
    lines.append("- If a structural row fails badly, it means this market still needs loser-side inventory accumulation to monetize rebounds, so a full rewrite must pair entry logic with a new hedge/allocator layer.")
    lines.append("- The key metric is whether `flush/reclaim + pilot fill` moves the frontier, not just whether it lowers MDD by destroying exposure.")
    lines.append("")
        ...
        rows.append(
            {
                "variant": cfg["variant"],
                "mode": cfg["mode"],
                "flush_drop_pct": float(cfg.get("flush_drop_pct", 0.0)),
                "reclaim_pct": float(cfg.get("reclaim_pct", 0.0)),
                "setup_wait_bars": int(cfg.get("setup_wait_bars", 0)),
                "pilot_entry_scale": float(cfg.get("pilot_entry_scale", ENTRY_SCALE)),
                "pyramid_step_pct": float(cfg.get("pyramid_step_pct", 0.0)),
                ...
                "setup_created": int(bt.stats.get("setup_created", 0)),
                "setup_flushed": int(bt.stats.get("setup_flushed", 0)),
                "setup_filled": int(bt.stats.get("setup_filled", 0)),
                "setup_cancelled": int(bt.stats.get("setup_cancelled", 0)),
                "pyramid_add_events": int(bt.stats.get("pyramid_add_events", 0)),
                "hard_stop_events": int(bt.stats.get("hard_stop_events", 0)),
                "lower_reentry_blocked": int(bt.stats.get("lower_reentry_blocked", 0)),
            }
        )
