...
OUT_BASE = "53_backtest_btcusdt_scale06_adx002_case1_m4_early_dca_brake_compare"
    ...
    class EarlyDcaBrakeCase(base_cls):
        ...
        def _check_stop_loss_and_reentry(self, price: float, timestamp):
            ...
            if re["side"] == "LONG" and price <= re["reentry_price"]:
                blocked, _ = self._brake_active(price, timestamp)
                if blocked:
                    self.stats["blocked_reentry_signals"] = int(self.stats.get("blocked_reentry_signals", 0)) + 1
                    return
                if self._add_to_position(re["reentry_price"], timestamp, re["quantity"], "REENTRY"):
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("53 Study: MaxEntries=4 Early Long DCA Brake + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 53 Backtest: MaxEntries=4 Early Long DCA Brake Variants")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Variant idea: keep `OPEN`/reverse logic intact, but block only `LONG DCA/REENTRY` when the position is already stressed under bearish or crash-like conditions.")
    lines.append("")
    ...
    if improved.empty:
        lines.append("- No tested early-brake variant achieved both `higher total CAGR` and `lower total MDD` than `baseline_m4`.")
    else:
    ...
    lines.append("- If a variant helps, it means the real problem was long inventory build-up rather than hedge-close timing.")
    lines.append("- If CAGR falls more than MDD improves, the brake is firing too early and is cutting profitable averaging paths.")
    lines.append("- The key test is whether narrowing the intervention to stressed long adds can improve both CAGR and MDD over the study-51 baseline.")
    lines.append("")
