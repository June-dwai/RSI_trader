...
OUT_BASE = "35_backtest_btcusdt_live_nla_longonly_hedge_hyst05"
...
def save_plot(eq_df: pd.DataFrame, signal_df: pd.DataFrame, events_df: pd.DataFrame):
    ...
    ax0.set_title("35 Study: Live-parity No-lookahead (Long-only + Trend Short Hedge, Hyst 0.5%)")
...
def save_report(
    ...
    lines: list[str] = []
    lines.append("# 35 Backtest: Live Parity (No-lookahead, Long-only + Trend Short Hedge Hyst 0.5%)")
    lines.append("")
    lines.append("## Setup")
