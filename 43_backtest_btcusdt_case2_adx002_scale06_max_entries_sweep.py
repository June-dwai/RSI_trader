...
OUT_BASE = "43_backtest_btcusdt_case2_adx002_scale06_max_entries_sweep"
...
def save_plot(equity_map: dict[int, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("43 Study: Case2(42) Sweep by Max Entries")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 43 Backtest: Study-42 Case2 Max Entries Sweep")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Strategy: case2 from study-42 only.")
    lines.append("- Engine traits: dual-direction, no short hedge, no hysteresis, ADX 002 logic, previous confirmed 4h touch only.")
    lines.append("- Initial capital per run: `1000 USDT`")
    lines.append("- Entry scale: `0.60`")
...
def run():
    base = load_module("m002_43", BASE_002_PATH)
    helper = load_module("m04_43", BASE_04_PATH)
    m32 = load_module("m32_43", BASE_32_PATH)
    m42 = load_module("m42_43", BASE_42_PATH)

    df_1m, df_4h = m32.load_data_no_filter(base)
    df_1m = df_1m[(df_1m.index >= base.BACKTEST_START) & (df_1m.index <= base.BACKTEST_END)].copy()

    Case2BaseClass = m42.build_case2_class(m32)
    ...
    for max_entries in MAX_ENTRIES_LIST:
        cls = build_max_entries_class(Case2BaseClass, max_entries)
        bt = cls(
            base_module=base,
            symbol=base.SYMBOL,
