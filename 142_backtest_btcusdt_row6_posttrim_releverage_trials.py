...
OUT_BASE = "142_backtest_btcusdt_row6_posttrim_releverage_trials"
...
def resolve_posttrim_cap(
    mode: str,
    bars_since_trim: int,
    price_close: float,
    red_avg: float,
    bearish_ob_above_count: int,
    base_leverage: float,
) -> tuple[float | None, bool]:
    if mode == "none":
        return None, False
    if mode == "same_regime_2p0":
        return 2.0, False
    if mode == "wait32_2p0":
        if bars_since_trim < 32:
            return 2.0, False
        return None, True
    if mode == "confirm32_2p0":
        if bars_since_trim < 32 or price_close <= red_avg or bearish_ob_above_count > 2:
            return 2.0, False
        return None, True
    if mode == "stage16_64":
        if bars_since_trim < 16:
            return 2.0, False
        if bars_since_trim < 64 or price_close <= red_avg or bearish_ob_above_count > 2:
            return min(2.5, base_leverage), False
        return None, True
    if mode == "stage16_96_strict":
        if bars_since_trim < 16:
            return 2.0, False
        if bars_since_trim < 96 or price_close <= red_avg * 1.005 or bearish_ob_above_count > 1:
            return min(2.5, base_leverage), False
        return None, True
    raise ValueError(f"Unsupported posttrim mode: {mode}")
...
def run_variant(df: pd.DataFrame, cfg: dict, s76, m117) -> tuple[pd.DataFrame, dict]:
    ...
    posttrim_mode = str(cfg["posttrim_mode"])
    ...
    stats = {
        ...
        "posttrim_capped_long_entries": 0,
        "posttrim_release_count": 0,
    }
    ...
    for i in range(len(df)):
        ...
        if not blocked_reentry and side != desired_side:
            ...
            if desired_side != 0 and wallet > 0:
                ...
                if allow_entry and desired_side > 0 and posttrim_active:
                    cap, released = resolve_posttrim_cap(
                        posttrim_mode,
                        i - posttrim_idx,
                        price_close,
                        float(red_avg[i]),
                    ...
                    if cap is not None and cap < lev_to_open:
                        lev_to_open = cap
                        stats["posttrim_capped_long_entries"] += 1
                    if released:
                        posttrim_active = False
                        posttrim_idx = -10**9
                        stats["posttrim_release_count"] += 1
...
def save_plot(curves_df: pd.DataFrame, metrics_df: pd.DataFrame) -> None:
    ...
    ax_eq.set_title("Study 142: Row6 Post-Bulltrim Re-Leverage Trials")
...
def save_report(metrics_df: pd.DataFrame, whipsaw_windows: pd.DataFrame, start_ts: pd.Timestamp, end_ts: pd.Timestamp) -> None:
    ...
    lines = []
    lines.append("# 142번 연구: row6 bulltrim 이후 재레버리지 제한 실험")
    lines.append("")
    lines.append("- 기준 엔진은 `138`의 실전형 베이스인 `combo_trim2p0_unlock24h`입니다.")
    lines.append(f"- 비교 구간은 `{start_ts}` ~ `{end_ts}`입니다.")
    lines.append("- `141`에서 generic chop filter가 잘 안 먹혔기 때문에, 이번엔 문제를 더 좁혀서 `bulltrim 이후 long이 너무 빨리 다시 3.0x로 복귀하는가`를 직접 테스트했습니다.")
    lines.append("- 해석 주의: inherited `slow_bear_bars=1440`은 15분봉 기준이므로 약 15일 지속 bearish 조건입니다.")
    lines.append("")
    lines.append("## 실험 아이디어")
    lines.append("- `bulltrim`은 bullish regime 안에서 이상 징후가 잡히면 기존 long을 닫고 `2.0x` long으로 다시 여는 완충 장치입니다.")
    lines.append("- 이번 연구는 그 이후 long 재진입이 다시 발생할 때, 곧바로 `3.0x`로 복귀시키지 말고 일정 조건이 충족될 때까지 낮은 레버리지를 유지하면 whipsaw를 줄일 수 있는지 보는 실험입니다.")
    lines.append("- 즉 `141`의 chop 전역 필터가 아니라, `trim 이후 long 쪽 재가속만 제어`하는 훨씬 국소적인 수정입니다.")
    lines.append("")
    lines.append("## Variant Table")
    lines.append("| Variant | CAGR % | MDD % | Calmar | 2026 Return % | 2026 MDD % | Whipsaw Avg Return % | Whipsaw Avg MDD % | Bull Trims | Unlocks | Slow Bear Shorts | Posttrim Capped Longs | Posttrim Releases |")
    lines.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
    for _, row in metrics_df.iterrows():
        lines.append(
            ...
            f"{int(row['posttrim_capped_long_entries'])} | {int(row['posttrim_release_count'])} |"
        )
    lines.append("")
    lines.append("## Variant Meaning")
    lines.append("- `combo_base`: `138` 실전형 그대로. trim 이후에도 다음 long 재진입은 바로 원래 레버리지로 복귀합니다.")
    lines.append("- `combo_posttrim_same_regime_2p0`: trim이 한 번 나오면 같은 bullish regime이 끝날 때까지 long은 계속 `2.0x`로 제한합니다.")
    lines.append("- `combo_posttrim_wait32_2p0`: trim 후 32개 15분봉 동안만 `2.0x` 유지하고 그 뒤엔 자동 해제합니다.")
    lines.append("- `combo_posttrim_confirm32_2p0`: 32개 봉 경과 + `red_avg` 위 회복 + 위쪽 bearish OB 완화가 동시에 확인될 때만 해제합니다.")
    lines.append("- `combo_posttrim_stage16_64`: 처음 16개 봉은 `2.0x`, 그다음은 `2.5x`로 완화하고 64개 봉 이후 조건이 좋아지면 풀레버리지 복귀를 허용합니다.")
    lines.append("- `combo_posttrim_stage16_96_strict`: 더 오래, 더 엄격하게 `2.0x -> 2.5x -> 3.0x` 단계 복귀를 강제하는 버전입니다.")
    lines.append("")
    lines.append("## 기준 대비 읽는 법")
    lines.append(f"- 기준 `combo_base`: CAGR `{_fmt(baseline['cagr_pct'])}%`, MDD `{_fmt(baseline['max_drawdown_pct'])}%`, Calmar `{_fmt(baseline['calmar_ratio'])}`, 2026 `{_fmt(baseline['return_2026_pct'])}%`, whipsaw 평균 수익률 `{_fmt(baseline['whipsaw_avg_return_pct'])}%`입니다.")
    lines.append(f"- 전체 균형 최상위: `{best_practical['variant']}` -> Calmar `{_fmt(best_practical['calmar_ratio'])}`, CAGR `{_fmt(best_practical['cagr_pct'])}%`, MDD `{_fmt(best_practical['max_drawdown_pct'])}%`.")
    lines.append(f"- whipsaw 방어 최상위: `{best_whipsaw['variant']}` -> whipsaw 평균 수익률 `{_fmt(best_whipsaw['whipsaw_avg_return_pct'])}%`, whipsaw 평균 MDD `{_fmt(best_whipsaw['whipsaw_avg_mdd_pct'])}%`.")
    lines.append("")
    lines.append("## 해석 포인트")
    lines.append("- 좋은 수정이라면 `whipsaw 평균 손실`이 먼저 개선되고, 그 다음에 CAGR 손상이 얼마나 작은지를 봐야 합니다.")
    lines.append("- `posttrim_capped_long_entries`가 많다는 건 trim 이후 long 재가속을 실제로 자주 눌렀다는 뜻입니다.")
    lines.append("- 그런데도 whipsaw가 개선되지 않으면, 문제는 `bulltrim 이후 재레버리지`보다 더 앞단의 진입/방향 판별에 있을 가능성이 큽니다.")
    lines.append("- 반대로 whipsaw가 좋아지는데 CAGR 훼손이 작다면, `141`보다 훨씬 실전적인 개선으로 볼 수 있습니다.")
    lines.append("")
    lines.append("## Whipsaw Windows Used")
    for _, row in whipsaw_windows.iterrows():
        lines.append(f"- `{row['peak_time']}` -> `{row['trough_time']}`")
    lines.append("")
    lines.append("## 실전성 판단 체크리스트")
    lines.append("- baseline 대비 CAGR을 지키면서 MDD 또는 whipsaw 손실을 줄였는가")
    lines.append("- 2026 방어가 유지되거나 좋아졌는가")
    lines.append("- 수정 효과가 `한두 번의 우연`이 아니라 `capped_long_entries`, `release_count` 같은 구조적 흔적으로 확인되는가")
    lines.append("- 너무 오래 `2.0x`에 묶여서 장기 상승장의 엔진을 망치지는 않는가")
    lines.append("")
    lines.append("## 산출물")
...
def main() -> None:
    m47 = load_module("study47_for_142", BASE_47_PATH)
    s76 = load_module("study76_for_142", BASE_76_PATH)
    m111 = load_module("study111_for_142", BASE_111_PATH)
    m114 = load_module("study114_for_142", BASE_114_PATH)
    m117 = load_module("study117_for_142", BASE_117_PATH)
    m126 = load_module("study126_for_142", BASE_126_PATH)
    ...
    trial_cfgs = [
        {**common_combo, "variant": "combo_base", "posttrim_mode": "none"},
        {**common_combo, "variant": "combo_posttrim_same_regime_2p0", "posttrim_mode": "same_regime_2p0"},
        {**common_combo, "variant": "combo_posttrim_wait32_2p0", "posttrim_mode": "wait32_2p0"},
        {**common_combo, "variant": "combo_posttrim_confirm32_2p0", "posttrim_mode": "confirm32_2p0"},
        {**common_combo, "variant": "combo_posttrim_stage16_64", "posttrim_mode": "stage16_64"},
        {**common_combo, "variant": "combo_posttrim_stage16_96_strict", "posttrim_mode": "stage16_96_strict"},
    ]
            ...
            whipsaw_rows.append(
                {
                    "variant": cfg["variant"],
                    "window_id": f"W{idx + 1}",
                    "peak_time": pd.Timestamp(win["peak_time"]),
                    "trough_time": pd.Timestamp(win["trough_time"]),
                    **seg,
                }
            )
    ...
    print(
        metrics_df[
            [
                ...
                "return_2026_pct",
                "mdd_2026_pct",
                "whipsaw_avg_return_pct",
                "whipsaw_avg_mdd_pct",
                "bulltrim_count",
                "posttrim_capped_long_entries",
                "posttrim_release_count",
            ]
