...
OUT_BASE = "56_backtest_btcusdt_scale06_adx002_case1_m4_dca_spacing_compare"
...
VARIANTS = [
    {"variant": "baseline_drop0p50", "dca_drop_pct": 0.0050},
    {"variant": "dca_drop0p75", "dca_drop_pct": 0.0075},
    {"variant": "dca_drop1p00", "dca_drop_pct": 0.0100},
    {"variant": "dca_drop1p25", "dca_drop_pct": 0.0125},
    {"variant": "dca_drop1p50", "dca_drop_pct": 0.0150},
]
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("56 Study: MaxEntries=4 DCA Spacing Variants + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    baseline = metrics_df[metrics_df["variant"] == "baseline_drop0p50"].iloc[0]
    improved = metrics_df[
    ...
    lines: list[str] = []
    lines.append("# 56 Backtest: MaxEntries=4 DCA Spacing Variants")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Variant idea: keep hedge behavior unchanged, but widen the required drop between long adds so the strategy does not reach full size too quickly.")
    lines.append("")
    lines.append("## Results")
    lines.append("")
    lines.append("| Variant | DCA Drop % | Total Final Equity | Total CAGR % | Total MDD % | Total Calmar | Case1 CAGR % | Case1 MDD % | DCA Signals | Hedge Top-up |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| `{r['variant']}` | {_fmt(r['dca_drop_pct'] * 100.0)} | "
    ...
    lines.append("")
    lines.append("## Delta vs baseline_drop0p50")
    ...
    if improved.empty:
        lines.append("- No tested DCA-spacing variant achieved both `higher total CAGR` and `lower total MDD` than `baseline_drop0p50`.")
    else:
    ...
    lines.append("- If a variant helps, it means the main problem was reaching full size too fast rather than the existence of the add path itself.")
    lines.append("- If CAGR improves while MDD also improves, deeper spacing is buying better average prices without killing too many rebounds.")
    lines.append("- The key metric is whether wider DCA spacing improves both total CAGR and total MDD over the `max_entries=4` baseline.")
    lines.append("")
...
def run():
    ...
    for cfg in VARIANTS:
        cls = build_variant_class(m47.LiveParityNoLookahead, cfg["dca_drop_pct"])
        bt = cls(
            symbol=m47.SYMBOL,
