...
OUT_BASE = "55_backtest_btcusdt_scale06_adx002_case1_m4_partial_dca_size_compare"
...
VARIANTS = [
    {
        ...
        "stress_add_fraction": 1.00,
    },
    {
        "variant": "bear4_entry3_gap0_add50",
        "mode": "bear_run",
        "min_bear_run_4h": 4,
        "min_entry_count": 3,
        "stress_gap_pct": 0.00,
        "risk_score_min": 0,
        "dd30_threshold_pct": -999.0,
        "stress_add_fraction": 0.50,
    },
    {
        "variant": "bear4_entry3_gap0_add25",
        "mode": "bear_run",
        "min_bear_run_4h": 4,
        "min_entry_count": 3,
        "stress_gap_pct": 0.00,
        "risk_score_min": 0,
        "dd30_threshold_pct": -999.0,
        "stress_add_fraction": 0.25,
    },
    {
        "variant": "risk4_entry2_gap0_add50",
        "mode": "risk",
        "min_bear_run_4h": 0,
        "min_entry_count": 2,
        "stress_gap_pct": 0.00,
    ...
    {
        "variant": "risk4_entry3_gap1_add50",
        "mode": "risk",
        "min_bear_run_4h": 0,
        "min_entry_count": 3,
        "stress_gap_pct": 0.01,
...
def build_variant_class(base_cls, cfg: dict):
    ...
    stress_add_fraction_value = float(cfg["stress_add_fraction"])
    ...
    class EarlyDcaBrakeCase(base_cls):
        ...
        stress_add_fraction = stress_add_fraction_value

        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.current_context_row = None
            self.stats["scaled_dca_signals"] = 0
            ...
            if self.current_position["side"] == "LONG":
                if price <= self.recent_trade[0] * 0.995:
                    ...
                    if mult > 0:
                        ...
                        add_qty = self.position_quantity * mult
                        tag = f"DCA_x{mult}"
                        if blocked:
                            add_qty *= self.stress_add_fraction
                            tag = f"{tag}_stress{self.stress_add_fraction:.2f}"
                            self.stats["scaled_dca_signals"] = int(self.stats.get("scaled_dca_signals", 0)) + 1
                        if add_qty > 0 and self._add_to_position(price, timestamp, add_qty, tag):
                            self.last_order_time = current_time
                return
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("55 Study: MaxEntries=4 Partial Stress DCA Size + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 55 Backtest: MaxEntries=4 Partial Stress DCA Size Variants")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Variant idea: keep `OPEN` and `REENTRY` intact, but scale down `LONG DCA` size when the position is already stressed under bearish or crash-like conditions.")
    lines.append("")
    lines.append("## Results")
    lines.append("")
    lines.append("| Variant | Mode | Min Bear Run 4h | Min Entry Count | Stress Gap % | Stress Add % | Total Final Equity | Total CAGR % | Total MDD % | Total Calmar | Case1 CAGR % | Case1 MDD % | Scaled DCA | Brake Hits |")
    lines.append("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| `{r['variant']}` | `{r['mode']}` | {int(r['min_bear_run_4h'])} | {int(r['min_entry_count'])} | {_fmt(r['stress_gap_pct'] * 100.0)} | {_fmt(r['stress_add_fraction'] * 100.0)} | "
            f"{_fmt(r['total_final_equity'])} | {_fmt(r['total_cagr_pct'])} | {_fmt(r['total_mdd_pct'])} | {_fmt(r['total_calmar_ratio'])} | "
            f"{_fmt(r['case1_cagr_pct'])} | {_fmt(r['case1_mdd_pct'])} | {int(r['scaled_dca_signals'])} | {int(r['brake_active_bars_in_position'])} |"
        )
    lines.append("")
    ...
    if improved.empty:
        lines.append("- No tested partial-size variant achieved both `higher total CAGR` and `lower total MDD` than `baseline_m4`.")
    else:
    ...
    lines.append("- If a variant helps, it means the real problem was the size of the stressed final add, not the existence of the add path itself.")
    lines.append("- If CAGR falls more than MDD improves, even partial scaling is still too blunt or triggers too early.")
    lines.append("- The key test is whether reducing stressed long add size can improve both CAGR and MDD over the study-51 baseline.")
    lines.append("")
        ...
        rows.append(
            {
                ...
                "stress_add_fraction": cfg["stress_add_fraction"],
                ...
                "scaled_dca_signals": int(bt.stats.get("scaled_dca_signals", 0)),
