...
OUT_BASE = "59_backtest_btcusdt_scale06_adx002_case1_m4_dca075_add_cooldown_compare"
...
VARIANTS = [
    {"variant": "baseline_cool0", "add_cooldown_bars": 0},
    {"variant": "addcool30", "add_cooldown_bars": 30},
    {"variant": "addcool60", "add_cooldown_bars": 60},
    {"variant": "addcool120", "add_cooldown_bars": 120},
    {"variant": "addcool240", "add_cooldown_bars": 240},
]
...
def build_variant_class(base_cls, add_cooldown_bars: int):
    add_cooldown_bars_value = int(add_cooldown_bars)

    class AddCooldownCase(base_cls):
        max_entries_cap = MAX_ENTRIES
        dca_drop_pct = DCA_DROP_PCT
        add_cooldown_bars = add_cooldown_bars_value

        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.stats["hedge_topup_events"] = 0
            self.stats["dca_signal_events"] = 0
            self.stats["dca_blocked_cooldown"] = 0
            self.last_long_add_idx: int | None = None
        ...
        def _process_long_entry(self, price: float, timestamp, adx: float, current_time: int):
            if not self.current_position:
                ...
                self.last_long_add_idx = current_time
                return
            ...
            if self.current_position["side"] == "LONG":
                ...
                if price <= trigger_price:
                    if (
                        self.add_cooldown_bars > 0
                        and self.last_long_add_idx is not None
                        and (current_time - self.last_long_add_idx) < self.add_cooldown_bars
                    ):
                        self.stats["dca_blocked_cooldown"] = int(self.stats.get("dca_blocked_cooldown", 0)) + 1
                        return
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("59 Study: DCA 0.75% + Add Cooldown Variants + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    baseline = metrics_df[metrics_df["variant"] == "baseline_cool0"].iloc[0]
    improved = metrics_df[
    ...
    lines: list[str] = []
    lines.append("# 59 Backtest: DCA 0.75% with Add Cooldown Variants")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Variant idea: keep the same price trigger for adds, but require extra time between long add events so full size cannot be built too quickly during one selloff.")
    lines.append("")
    lines.append("## Results")
    lines.append("")
    lines.append("| Variant | Add Cooldown Bars | Total Final Equity | Total CAGR % | Total MDD % | Total Calmar | Case1 CAGR % | Case1 MDD % | DCA Signals | DCA Blocked | Hedge Top-up |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| `{r['variant']}` | {int(r['add_cooldown_bars'])} | "
            ...
            f"{int(r['dca_signal_events'])} | {int(r['dca_blocked_cooldown'])} | {int(r['hedge_topup_events'])} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append("## Delta vs baseline_cool0")
    ...
    if improved.empty:
        lines.append("- No tested add-cooldown variant achieved both `higher total CAGR` and `lower total MDD` than `baseline_cool0`.")
    else:
    ...
    lines.append("- If a variant helps, it means position size is building too quickly in clock time, not just at the wrong price gaps.")
    lines.append("- Add cooldown is a softer structural brake than hard DCA blocking because it delays clustering without deleting the add path entirely.")
    lines.append("- The key metric is whether slower inventory pacing can keep the study-56 CAGR gain while reducing total MDD.")
    lines.append("")
...
def run():
    ...
    for cfg in VARIANTS:
        cls = build_variant_class(m47.LiveParityNoLookahead, cfg["add_cooldown_bars"])
        bt = cls(
            symbol=m47.SYMBOL,
        ...
        rows.append(
            {
                "variant": cfg["variant"],
                "add_cooldown_bars": cfg["add_cooldown_bars"],
                ...
                "dca_blocked_cooldown": int(bt.stats.get("dca_blocked_cooldown", 0)),
