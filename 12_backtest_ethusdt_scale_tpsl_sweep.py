...
SCRIPT_FILE = Path("12_backtest_ethusdt_scale_tpsl_sweep.py")
PLOT_FILE = Path("12_backtest_ethusdt_scale_tpsl_sweep.png")
CSV_FILE = Path("12_backtest_ethusdt_scale_tpsl_sweep.csv")
MD_FILE = Path("12_backtest_ethusdt_scale_tpsl_sweep.md")

TARGET_SYMBOL = "ETHUSDT"
FIXED_HYSTERESIS = 0.0  # Fix hysteresis to isolate scale/TP/SL effects.

ENTRY_SCALES = [0.30, 0.40]
TAKE_PROFIT_VALUES = [0.006, 0.008]  # 0.6%, 0.8%
STOP_LOSS_VALUES = [0.040, 0.050, 0.060, 0.080]  # 4.0%, 5.0%, 6.0%, 8.0%
...
class SweepCase:
    ...
    def case_id(self) -> str:
        return (
            f"s{self.entry_scale:.2f}_tp{self.take_profit_pct * 100:.2f}_sl{self.stop_loss_pct * 100:.2f}"
        )
...
def run_case(
    ...
    cls = helper_08.build_fixed5x_hyst_class(base_module, helper_04, FIXED_HYSTERESIS)
    bt = cls(
    ...
    print(
        ...
        f"(scale={case.entry_scale:.2f}, tp={case.take_profit_pct * 100:.2f}%, sl={case.stop_loss_pct * 100:.2f}%)"
    )
...
def save_csv(results: list[CaseResult]) -> pd.DataFrame:
...
def save_plot(results: list[CaseResult]):
    ...
    ax_eq.set_title("12 ETHUSDT Sweep: Top 4 Equity Cases (Scale + TP + SL)")
    ...
    for i, r in enumerate(top_results):
        ...
        if r.equity.empty:
            continue
        ax_eq.plot(r.equity["timestamp"], r.equity["equity"], label=label, linewidth=1.2, color=colors[i % len(colors)])
    ax_eq.legend(loc="upper left", fontsize=9)
    ax_eq.grid(True, alpha=0.2)

    ax_final = fig.add_subplot(gs[1, 0])
    ax_final.set_title("Top 4 Final Equity")
    ax_final.bar(labels, [r.summary["final_equity"] for r in top_results], color=colors[: len(top_results)])
    ...
    ax_mdd.bar(labels, [r.summary["max_drawdown_pct"] for r in top_results], color=colors[: len(top_results)])
...
def save_md(results: list[CaseResult], metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 12 ETHUSDT Scale + TP/SL Case Study")
    lines.append("")
    lines.append("## 1) Objective")
    lines.append("- Explore ETH tuning sensitivity for `entry_scale`, `TP`, and `SL` only.")
    lines.append("- Keep hedge method fixed to `04 trend short hedge 5x` with 4h confirmed trend.")
    lines.append(f"- Keep hysteresis fixed at `{FIXED_HYSTERESIS * 100:.2f}%` to isolate parameter effects.")
    lines.append("")
    ...
    lines.append(f"- Scale values: `{', '.join([f'{s:.2f}' for s in ENTRY_SCALES])}`")
    ...
    lines.append("")
    lines.append("| Rank | Case | Scale | TP % | SL % | Final Equity | Return % | CAGR % | MDD % | Calmar | Trades | Win Rate % |")
    lines.append("|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for i, r in enumerate(top4, start=1):
        s = r.summary
        lines.append(
            f"| {i} | `{r.case.case_id}` | {_fmt(s['entry_scale'], 2)} | {_fmt(s['take_profit_pct'] * 100)} | "
    ...
    lines.append("")
    lines.append("| Rank | Case | Scale | TP % | SL % | Final Equity | Return % | CAGR % | MDD % | Calmar | Trades | Long/Short | Win Rate % | Profit Factor | Worst Month |")
    lines.append("|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for i, r in enumerate(sorted_results, start=1):
        s = r.summary
        lines.append(
            f"| {i} | `{r.case.case_id}` | {_fmt(s['entry_scale'], 2)} | {_fmt(s['take_profit_pct'] * 100)} | {_fmt(s['stop_loss_pct'] * 100)} | "
    ...
    for i, r in enumerate(sorted_results, start=1):
        ...
        lines.append(
            f"- Params: scale `{_fmt(s['entry_scale'], 2)}`, TP `{_fmt(s['take_profit_pct'] * 100)}%`, "
            f"SL `{_fmt(s['stop_loss_pct'] * 100)}%`, hysteresis `{_fmt(s['fixed_hysteresis'] * 100)}%`"
        )
...
def run():
    base_module = load_module("m002_12", BASE_002_PATH)
    helper_04 = load_module("m04_12", BASE_04_PATH)
    helper_08 = load_module("m08_12", BASE_08_PATH)
    ...
    cases = [
        SweepCase(entry_scale=s, take_profit_pct=tp, stop_loss_pct=sl)
        for s in ENTRY_SCALES
