...
OUT_BASE = "30_backtest_btcusdt_02_both_max_entries_sweep"
...
def save_plot(equity_map: dict[int, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.axhline(INITIAL_CAPITAL, color="#777777", linestyle="--", linewidth=0.9, label="Start 500")
    ax_eq.set_title("30 Study: 02 Both-sides Sweep by Max Entry Count")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 30 Backtest: 02 Both-sides Max Entries Sweep")
    lines.append("")
    lines.append("## Setup")
    lines.append("- Base: 02 both-sides (long/short entries active), no-lookahead confirmed 4h touch")
...
def run():
    base = load_module("m002_30", BASE_002_PATH)
    helper = load_module("m04_30", BASE_04_PATH)
    ...
    for max_entries in MAX_ENTRIES_LIST:
        cls = build_max_entries_class(base.FloorScaledRSIAveragingBacktest, max_entries)
        bt = cls(
            symbol=base.SYMBOL,
