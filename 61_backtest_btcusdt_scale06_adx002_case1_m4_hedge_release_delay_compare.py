...
OUT_BASE = "61_backtest_btcusdt_scale06_adx002_case1_m4_hedge_release_delay_compare"
...
VARIANTS = [
    ...
    {"variant": "shallow2_else2bull", "bullish_close_bars": 2, "shallow_gap_pct": 0.02},
    {"variant": "shallow5_else2bull", "bullish_close_bars": 2, "shallow_gap_pct": 0.05},
    {"variant": "release2bull", "bullish_close_bars": 2, "shallow_gap_pct": -1.0},
]
...
def save_plot(curve_map: dict[str, pd.DataFrame], metrics_df: pd.DataFrame):
    ...
    ax_eq.set_title("61 Study: MaxEntries=4 Baseline + Hedge Release Delay Variants + Fixed Case2")
...
def save_report(metrics_df: pd.DataFrame):
    ...
    lines: list[str] = []
    lines.append("# 61 Backtest: MaxEntries=4 Baseline with Hedge Release Delay Variants")
    lines.append("")
    lines.append("## Setup")
    ...
    lines.append("- Variant idea: keep bearish hedge open longer on bullish reversals instead of closing on the first confirmed bullish 4h bucket.")
    lines.append("")
    ...
    lines.append("- The key metric is whether smarter hedge release can improve the original `max_entries=4` total portfolio rather than only helping the more aggressive `dca0.75` branch.")
    lines.append("")
